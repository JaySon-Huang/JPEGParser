...
public class OtherDecoder {
    ...
    private ScanHeader mScanHeader = new ScanHeader();
    ...
    class ScanComponent {
        int comp_id, //Scan component selector
                dc_huffman, //DC table selector
                ac_huffman; //AC table selector
    }


    class ScanHeader {
        int Ls, //length
                componets_num, //Number of components in the scan
                s_start, //Start of spectral or predictor selection
                s_end, //End of spectral selection
                Ah,
                Al;

        ScanComponent Comp[]; //Components comp_id Td ac_huffman
        // from [0] to [componets_num-1]
        int get(InputStream in) throws Exception {
            ...
            Ls = get16(in);
            count += 2;
            componets_num = get8(in);
            count++;
            Comp = new ScanComponent[componets_num];
            for (i = 0; i < componets_num; i++) {
                Comp[i] = new ScanComponent();
                if (count > Ls) {
                    error("ERROR: scan header format error");
                }
                Comp[i].comp_id = get8(in);
                count++;
                temp = get8(in);
                count++;
                Comp[i].dc_huffman = temp >> 4;
                Comp[i].ac_huffman = temp & 0x0F;
            }
            s_start = get8(in);
            count++;
            s_end = get8(in);
            ...
            Al = temp & 0x0F;
            if (count != Ls) {
                error("ERROR: scan header format error [count!=componets_num]");
            }
            return 1;
        }
    }
...
// return signed integer or 0x00FF??00 if it sees a MARKER
    ...
    private int decode_MCU(InputStream in, int PrevDC[],
        ...
        for (comp_id = 0; comp_id < nComp; comp_id++) {
            qtab = qTab[comp_id];
            actab = acTab[comp_id];
            dctab = dcTab[comp_id];

            for (int i = 0; i < nBlock[comp_id]; i++) {
                switch (comp_id)
                {
                    case 0:
                        System.out.print(" Y:");
                ...
                int t = getn(in, value, temp, index);
                PrevDC[comp_id] = IDCT_Source[0] = PrevDC[comp_id] + t;
                System.out.print(String.format("DC:%3d",IDCT_Source[0]));
                IDCT_Source[0] *= qtab[0];
                System.out.print(" AC:");
                for (int j = 1; j < 64; j++) {
                    value = HuffmanValue(actab, temp, index, in);
                    if (value >= 0xFF00) {
                        System.out.println("Mark happened!");
                        return value;
                    }
                    j += (value >>> 4);
                    if ((value & 0x0F) == 0) {
                        ...
                        int t2 = getn(in, value & 0x0F, temp, index);
                        System.out.print(String.format(", 0x%02x:(%2d, %2d)",value,value>>>4,t2));
                        IDCT_Source[IDCT_P[j]] =
                                 t2 * qtab[j];
                    }
                }
                System.out.println("END");
                ScaleIDCT(DU[comp_id][i]);
            }
        }
        return 0;
    }
...
// output: matrix
    ...
    public void decode(InputStream in, PixelArray out) throws Exception {
        ...
        do {
            ...
            // SOS block
            mScanHeader.get(in);
            nComp = (int) mScanHeader.componets_num;
            for (i = 0; i < nComp; i++) {
                int CompN = mScanHeader.Comp[i].comp_id;
                qTab[i] = QT.Q[FH.Comp[CompN].Tq];
                nBlock[i] = FH.Comp[CompN].V * FH.Comp[CompN].H;
                dcTab[i] = HuffTab[mScanHeader.Comp[i].dc_huffman][0];
                acTab[i] = HuffTab[mScanHeader.Comp[i].ac_huffman][1];
            }
            YH = FH.Comp[1].H;
            YV = FH.Comp[1].V;
            Xsize = FH.X;
